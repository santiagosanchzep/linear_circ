...
pheno <- pheno %>%
  mutate(Mutation = ifelse(Status_time== "1" & is.na(Mutation), "noncarrier", Mutation),
...
write.csv(dds.res, "/Mutation_noncarrier.csv")
...
volcano.name <- "PPMI Linear RNA Expression Last Visit ca/co + Non carriers"
l2f.lim <- 0
plim <- 0.05
choice <- 'padj'
